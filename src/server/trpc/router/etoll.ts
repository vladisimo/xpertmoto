 ...
 * test the business rules without spinning up a tRPC context.
 */
...
export const etollRouter = createTRPCRouter({
  listAccounts: managerProcedure.query(({ ctx }) =>
    ctx.prisma.etollAccount.findMany({
  ...
  createAccount: adminProcedure
    ...
    .mutation(({ ctx, input }) => {
      ...
      return ctx.prisma.etollAccount.create({
        data: {
          name: input.name,
          username: input.username,
          loginUrl: input.loginUrl ?? "https://account.myetoll.transport.nsw.gov.au/login",
          passwordEnc: sec.enc,
          passwordIv: sec.iv,
          passwordTag: sec.tag,
        },
  ...
  updateAccount: adminProcedure
    ...
    .mutation(({ ctx, input }) => {
      ...
      return ctx.prisma.etollAccount.update({
        where: { id: input.id },
        data,
        select: accountSelect,
      });
  ...
  deleteAccount: adminProcedure
    ...
    .mutation(({ ctx, input }) =>
      ctx.prisma.etollAccount.delete({ where: { id: input.id } }),
    ),
  ...
  runSyncNow: managerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        return await runEtollSync(ctx.prisma, input.id);
      } catch (e) {
        throw new TRPCError({
  ...
  listUnmatched: managerProcedure
    ...
    .query(async ({ ctx, input }) => {
      const where: Prisma.EtollUnmatchedRowWhereInput = {};
      ...
      if (input.search) {
        where.OR = [
          ...
          { concession: { contains: input.search, mode: "insensitive" } },
          { gantryCode: { contains: input.search, mode: "insensitive" } },
          { rawDetails: { contains: input.search, mode: "insensitive" } },
        ];
      }

      const items = await ctx.prisma.etollUnmatchedRow.findMany({
        where,
  ...
  unmatchedCount: managerProcedure
    ...
    .query(({ ctx, input }) =>
      ctx.prisma.etollUnmatchedRow.count({
        where: {
          resolvedAt: null,
  ...
  suggestBookingsForUnmatched: managerProcedure
    ...
    .query(async ({ ctx, input }) => {
      const row = await ctx.prisma.etollUnmatchedRow.findUnique({
        where: { id: input.id },
        select: { eventAt: true },
      });
      if (!row) throw new TRPCError({ code: "NOT_FOUND", message: "Unmatched row not found" });

      // Exact active window match first, then a ±3 day fuzzy window so staff
      // can still see recent bookings if the toll posted slightly off the
      // actual rental window (data-entry drift, late check-in, etc.).
  ...
  resolveUnmatched: managerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // If staff picked a booking, enforce that it matches the chosen vehicle
      // and derive customerId from it so the two stay consistent.
      ...
      if (existing) {
        await ctx.prisma.etollUnmatchedRow.update({
          where: { id: row.id },
          data: {
            resolvedAt: new Date(),
      ...
      // Match the auto-sync path: store the configured admin fee on the
      // infringement so the downstream Payment line items reflect it.
      ...
      const infringement = await ctx.prisma.infringement.create({
        data: {
          ...
          type: "TOLL",
          issuer: `NSW E-Toll (${row.account.name})`,
          ...
          notes: `Toll: ${row.concession}${row.gantryCode ? ` (${row.gantryCode})` : ""}. Source: ${row.rawDetails ?? row.rego}`,
        },
      });
      ...
      // Mirror the auto-sync side-effects when the staff member linked
      // the row to a customer: create the Payment, increment
      // booking.balanceDue, issue the AdjustmentNote.
      if (customerId) {
  ...
  dismissUnmatched: managerProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      return ctx.prisma.etollUnmatchedRow.update({
        where: { id: row.id },
        data: {
          resolvedAt: new Date(),
          resolvedById: ctx.session.user.id,
  ...
  listTransactions: managerProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.accountId) {
        const account = await ctx.prisma.etollAccount.findUnique({
          where: { id: input.accountId },
          select: { name: true },
        });
        if (!account) throw new TRPCError({ code: "NOT_FOUND", message: "Account not found" });
        where.issuer = { contains: `(${account.name})` };
      }
