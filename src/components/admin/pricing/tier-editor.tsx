...
export function TierEditor({ scope, scopeId, scopeLabel }: TierEditorProps) {
  ...
  // the shape of the ladder as they type. Derived from watched values.
  ...
  const addTier = () => {
    ...
    append({ minDays: nextMin, maxDays: nextMax, tierTotal: 0, isActive: true });
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="text-sm text-muted-foreground">
              {scope === "CATEGORY" ? "Category" : "Vehicle"} ladder
            </div>
        ...
        {isLoading ? (
          ...
          <div className="rounded-md border border-dashed border-border bg-muted/40 p-6 text-center">
            <p className="text-sm text-foreground">
              No tiers configured. This{" "}
              {scope === "CATEGORY" ? "category" : "vehicle"} uses the flat
          ...
          <div className="space-y-3">
            {fields.map((field, i) => (
              <div
                key={field.id}
                className="grid items-end gap-3 rounded-md border border-border bg-card p-3 md:grid-cols-[6rem_1rem_6rem_10rem_1fr_auto]"
              >
                <FormField
                  control={form.control}
