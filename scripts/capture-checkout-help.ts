/**
 * One-off (NOT wired into CI): capture real screenshots of the staff check-out
 * flow for the Help Centre "Checking a vehicle out" article.
 *
 * Auth: same throwaway-user recipe as capture-dashboard-help.ts — seed e2e
 * users don't exist in this dev DB and every real back-office user has TOTP,
 * so we create a clearly-labelled THROWAWAY staff user, mint a NextAuth v5
 * session cookie with AUTH_SECRET, shoot the pages, then delete the user in a
 * finally block.
 *
 * Run: npx tsx scripts/capture-checkout-help.ts
 */
...
const OUT = path.resolve(__dirname, "../public/help/check-out-workflow");
...
async function main() {
  ...
  // A CONFIRMED booking with a vehicle assigned drives the check-out hub.
  const booking = await prisma.booking.findFirst({
    where: { status: "CONFIRMED", vehicleId: { not: null } },
    orderBy: { pickupDateTime: "asc" },
    select: { id: true, bookingReference: true },
  });
  if (!booking) throw new Error("No CONFIRMED booking with a vehicle to screenshot");
  console.log("→ booking", booking.bookingReference, booking.id);

  const SHOTS = [
    { url: `${BASE}/staff/bookings/${booking.id}/check-out`, file: "overview.png" },
    { url: `${BASE}/staff/bookings/${booking.id}/check-out/inspect`, file: "inspect.png" },
  ];
